...
import type { ProjectTimeData } from '../../lib/analytics/timeAnalytics';
...
const FALLBACK_COLORS = [
  '#0ea5e9', '#6366f1', '#10b981', '#f59e0b', '#ef4444', '#8b5cf6', '#ec4899', '#14b8a6',
];

export function TimeByProjectChart({ data }: Props) {
  const visible = data.filter((d) => d.actualMinutes > 0).slice(0, 10);

  if (visible.length === 0) {
    return (
      <div className="bg-white rounded-xl border border-stone-200 p-5">
        <h3 className="text-sm font-semibold text-stone-700 mb-4">Time by Project</h3>
        <AnalyticsEmptyState message="No tracked time by project in this range." />
      </div>
    );
  }

  const chartData = visible.map((d) => ({
    name: d.projectName.length > 20 ? d.projectName.slice(0, 18) + '…' : d.projectName,
    fullName: d.projectName,
    minutes: d.actualMinutes,
    color: d.projectColor ?? FALLBACK_COLORS[0],
  ...
  return (
    <div className="bg-white rounded-xl border border-stone-200 p-5">
      ...
      <div className="mt-4 space-y-1.5 border-t border-stone-100 pt-4">
        {visible.map((item, i) => (
          <div key={item.projectId ?? 'none'} className="flex items-center gap-2 text-sm">
            <div
              className="w-2.5 h-2.5 rounded-full flex-shrink-0"
              style={{ backgroundColor: item.projectColor ?? FALLBACK_COLORS[i % FALLBACK_COLORS.length] }}
            />
            <span className="flex-1 text-stone-700 truncate">{item.projectName}</span>
